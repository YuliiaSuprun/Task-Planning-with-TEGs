...
public:
    ...
    virtual const std::vector<GridAction>& get_actions(const DomainState& state) const = 0;
    ...
    virtual const std::vector<std::pair<std::shared_ptr<DomainState>, GridAction>> get_successor_state_action_pairs(const DomainState& state) = 0;
